...
def run_generations(gen_count, state, rule_dict):
    ...
    for gen in range(gen_count):
        last_sum = plant_pos_sum(new_state, state_index)
        (new_state, state_index) = run_generation(new_state, state_index, rule_dict)
        temp = plant_pos_sum(new_state, state_index) - last_sum
        if difference == temp:
            return ((gen_count-gen)*difference)+last_sum
        else:
            difference = temp
    last_sum = plant_pos_sum(new_state, state_index)
    return last_sum
...
def main():
    GEN_COUNT = 50000000000
    lines = input("input.txt")
    (initial_state, rule_dict) = parse_text(lines)
    last_gen_sum = run_generations(GEN_COUNT, initial_state, rule_dict)
    output("output.txt", str(last_gen_sum))
